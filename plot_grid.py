...
if option1=='FVCOM':
    ...
    if option2=='GOM3':
        ...
        for i in range(len(lon)):
            ...
            for j in range(len(node_s[i])):
                if node_s[i][j]!=-1 and node_s[i][j]!=i:  #-1 means not a node,i means itself.
        ...
        for i in range(len(lon)):
            for j in range(len(lons[i])):
                plt.plot([lon[i],lons[i][j]],[lat[i],lats[i][j]],color='r')  #plot grid
    ...
    if option2=='MASSBAY':
        ...
        lonsize=[np.amin(lon_m), np.amax(lon_m)]
        latsize=[np.amin(lat_m), np.amax(lat_m)]   #range of massbay
        fig=plt.figure()
        ax=fig.add_subplot(111)
